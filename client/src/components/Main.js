...
import { updateItemInfoModal, areTwoArrSame } from ".././actions/booksAppActions";
...
class Main extends Component {
  ...
  shouldComponentUpdate(newProps, nextState) {
    if (!this.props.dispatch(areTwoArrSame(this.props.booksAppStore.booksApiData, newProps.booksAppStore.booksApiData)) || (this.props.booksAppStore.fetched !== newProps.booksAppStore.fetched) || (this.props.match.path !== newProps.match.path) || (this.props.booksAppStore.shownBooksLength !== newProps.booksAppStore.shownBooksLength)) {
      return true;
    } {
      return false;
    }
  ...
  renderResults = () => {
    if (this.props.booksAppStore.fetched) {
      return <Results />
    } else {
  ...
  renderResultsHeader = () => {
    return (
      <div className="results-header">
        <div className="results-count">Showing {this.props.booksAppStore.shownBooksLength} results</div>
        <SortBy />
      </div>
    )
  }
